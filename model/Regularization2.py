...
def L1_gradient(x,y,w,b,lamda):
  ...
  for i in range(m):
    ...
    dj_db=dj_db+err
  for i in range(n):
    dj_dw[i]=dj_dw[i]+np.sign(w[i])*lamda/2
  dj_dw=dj_dw/m
  dj_db=dj_db/m
...
def L1_gradient_descent(x,y,w_in,b_in,alpha,iterations,lamda):
  w=copy.deepcopy(w_in)
  b=copy.deepcopy(b_in)

  for i in range(iterations):
    djw,djb=L1_gradient(x,y,w,b,lamda)
    w=w-alpha*djw
    b=b-alpha*djb

  return w,b

def L1_cost_function(x,y,w,b,lamda):
  m=x.shape[0]
  cost=0.0
  reg_cost=0.0
  for i in range(m):
...
print(f"(w,b)=({w},{b})")
print(f"Cost={L1_cost_function(x_normal,ytrain,w,b,lamda)}")
print(f"After gradient descent")
w,b=L1_gradient_descent(x_normal,ytrain,w,b,alpha,1000,lamda)
print(f"(w,b)=({w},{b})")
print(f"Cost={L1_cost_function(x_normal,ytrain,w,b,lamda)}")
